    ...
    if (socket.IsNotNull()) // if client connected
      ...
      for (;;)
        ...
        if (strcmp(headerMsg->GetDeviceType(), "STT_TDATA") == 0)
          ...
          bool timeout(false);
          igtlUint64 r2 = socket->Receive(startTracking->GetPackBodyPointer(), startTracking->GetPackBodySize(), timeout);
...
void* ThreadFunction(void* ptr)
{
  ...
  // Send a RTS_TDATA message
  igtl::RTSTrackingDataMessage::Pointer rtsTrackingMsg;
  rtsTrackingMsg = igtl::RTSTrackingDataMessage::New();
  rtsTrackingMsg->SetDeviceName("TDataServer");
  rtsTrackingMsg->SetStatus(igtl::RTSTrackingDataMessage::STATUS_SUCCESS);
  rtsTrackingMsg->Pack();
  socket->Send(rtsTrackingMsg->GetPackPointer(), rtsTrackingMsg->GetPackSize());
